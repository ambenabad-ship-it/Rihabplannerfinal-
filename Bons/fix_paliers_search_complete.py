# -*- coding: utf-8 -*-
"""
Idempotent: applies the remaining JS changes for the search bar +
expanded product detail. Writes only at the end so a partial failure
is detectable but never leaves a half-saved file.
"""
import io, sys
...
def go(s, old, new, label):
    ...
    if o not in s:
        print('  [FAIL] ' + label); fails.append(label); return s
    if s.count(o) != 1:
        print('  [FAIL] ' + label + ' (count ' + str(s.count(o)) + ')'); fails.append(label); return s
    print('  [ok]   ' + label)
    return s.replace(o, n, 1)


# 1) Filter step right after the sort.
src = go(src,
    "  myPhones.sort((a, b) => {\n"
    ...
    "    return na.localeCompare(nb);\n"
    "  });\n"
    "  // Search filter — persistent across re-renders.\n"
    ...
    "      })\n"
    "    : myPhones;\n",
    'filter step (myPhonesFiltered)')

# 2) Replace expanded product list with delivered detail.
src = go(src,
    ...
    "            })()}\n"
    "          </div>\n",
    'expanded list shows per-product delivered qty + MAD')

# 3) Replace the iteration line + add empty-state inside the cards loop.
src = go(src,
    ...
    'iterate myPhonesFiltered + empty state')

# 4) Replace the header to include the search input + filtered/total count.
src = go(src,
    "  panel.innerHTML = `\n"
    ...
    "    ${cardsHtml}`;\n",
    'header with search input + filtered count')

# 5) Wire the search input handler before the expand-toggle wiring.
src = go(src,
